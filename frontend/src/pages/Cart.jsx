      ...
      <div className="container justify-around items-start flex wrap mx-auto mt-8">
        {cartItems.length === 0 ? (
          <div className="text-xl">
            ตะกร้าของคุณว่างเปล่า <Link className="text-2xl text-orange-400 underline" to="/shop">ไปดูสินค้าที่นี่</Link>
          </div>
        ) : (
          <>
              ...
              <div className="mt-8 w-[40rem]">
                <div className="p-4 rounded-lg">
                  <h2 className="text-xl font-semibold mb-2">
                    รายการทั้งหมด ({cartItems.reduce((acc, item) => acc + item.qty, 0)})
                  </h2>

                  <div className="text-2xl font-bold">
                    {" "}
                    {cartItems.reduce((acc, item) => acc + item.qty * item.price, 0).toFixed(2)} บาท
                  </div>

                  <button
                    className="bg-orange-500 mt-16 py-2 px-4 rounded-full text-lg w-full"
